...
export class ObjectStorageService {
  ...
  async downloadObject(file: File, res: Response, cacheTtlSec: number = 3600, customFilename?: string) {
    try {
      // Get file metadata
      const [metadata] = await file.getMetadata();
      
      // Use custom filename if provided, otherwise use object path
      let contentDisposition = `attachment; filename="${metadata.name}"`;
      if (customFilename) {
        // Create ASCII-safe fallback and UTF-8 encoded filename for UTF-8 support
